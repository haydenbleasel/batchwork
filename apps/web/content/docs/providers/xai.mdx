...
xAI image models (e.g. `grok-imagine-image-quality`) batch through `/v1/images/generations`, uploaded and polled the same way as chat. Submit with [`batchImages()`](/images):
...
const job = await batchImages({
  model: xai.imageModel("grok-imagine-image-quality"),
...
<Callout type="warn">
  xAI batch returns **signed image URLs** on `result.images[].url` (and/or
  inline `data`) that **expire ~1h** after the batch completes — fetch and
  persist them promptly. See [Images](/images).
</Callout>
...
xAI exposes no embedding model, so [`batchEmbeddings()`](/embeddings) throws `UnsupportedProviderError`.
