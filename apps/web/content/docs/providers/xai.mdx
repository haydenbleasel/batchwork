...
xAI image models (e.g. `grok-imagine-image-quality`) batch through `/v1/images/generations`, uploaded and polled the same way as chat. Submit with [`batch.images()`](/images):
...
xAI exposes no embedding model, so [`batch.embeddings()`](/embeddings) throws `UnsupportedProviderError`.
