...
It follows the [OpenAI](/providers/openai) lifecycle exactly: build `{ custom_id, method, url, body }` JSONL lines, upload via the Files API (`purpose=batch`), create the batch, poll `status`, then download the output/error files.
...
Groq has no native batch webhook, so it is poll-only. Use the [server layer](/server) for unified completion events.

Groq exposes no image or embedding model, so [`batch.images()`](/images) and [`batch.embeddings()`](/embeddings) throw `UnsupportedProviderError`.
