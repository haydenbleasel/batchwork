...
Anthropic exposes no image or embedding model, so [`batchImages()`](/images) and [`batchEmbeddings()`](/embeddings) throw `UnsupportedProviderError` before any network request. Batch chat completions are fully supported.
