...
4. **Read** — the JSONL at `results_url` is streamed and parsed into [`BatchResult`](/results)s.
...
Anthropic exposes no image or embedding model, so [`batch.images()`](/images) and [`batch.embeddings()`](/embeddings) throw `UnsupportedProviderError` before any network request. Batch chat completions are fully supported.

## Webhooks

Anthropic has no native batch webhook, so it is **poll-only**. Use the [server layer](/server)'s managed poller to get one unified, signed event when a batch finishes — the same event shape as every other provider.
