...
Mistral's `mistral-embed` model batches through the same job flow — the model is set on the job, and each request embeds one `value`. Submit with [`batch.embeddings()`](/embeddings):
...
The vector lands on `result.embedding`. Mistral has no image model, so [`batch.images()`](/images) throws `UnsupportedProviderError`.
