...
Together's batch API is chat/audio only — it rejects the embeddings endpoint and offers no batch image generation, so [`batch.embeddings()`](/embeddings) and [`batch.images()`](/images) throw `UnsupportedProviderError`.
