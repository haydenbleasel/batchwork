...
These options sit on the batch call alongside `model` and `requests`, and apply to **every** batch function — [`batch()`](/text), [`batch.images()`](/images), and [`batch.embeddings()`](/embeddings) — regardless of the per-request fields documented on each API page.
