...
`store` and `onComplete` are the only required options.
...
```json
{ "checked": 3, "delivered": ["batch_abc"], "failed": [] }
```
...
See [Server](/server) for the lower-level poller and the webhook-based flow this builds on.
