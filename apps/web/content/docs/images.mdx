---
title: Images
description: Generate thousands of images in one batch with batchImages — the same job handle, base64 or hosted images per request.
---

## Submit an image batch

`batchImages()` mirrors [`batch()`](/usage): pass an image model and a list of `prompt`s, and it returns the same `BatchJob` handle **immediately**. Each request produces one or more images, correlated by `customId`.
...
Everything on the [job handle](/job) works unchanged — `wait()`, `poll()`, `results()`, `collect()`, and `cancel()` — as does [rehydration](/usage#rehydrate-later) with `getBatch` / `getBatchResults` / `cancelBatch`.

You can also pass a `"provider/model"` string (e.g. `"openai/gpt-image-1"`), though the model object is recommended.
...
| ------------- | ---------------------------- | --------------------------------------------------------------- |
| OpenAI        | `gpt-image-1`                | `/v1/images/generations` batch endpoint. Inline base64.         |
...
<Callout type="warn">
  ...
  batch-supported. Calling `batchImages()` with any of them throws
...
`limits` and `metadata` work exactly as in [`batch()`](/requests#limits).
...
`defaults` are merged into every request (request-level values win), exactly as in [`batch()`](/requests#batch-wide-defaults) — handy for applying one `size` or `providerOptions` across the whole batch.

## Results

Images reuse the normalized [`BatchResult`](/results). Generated images land on `result.images`, an array of `{ data?, mediaType?, url? }`:
...
Like [`batch()`](/models#how-the-body-is-built), `batchImages()` derives each provider request body by running the AI SDK's `generateImage()` through a **capturing `fetch`** that records the serialized body and aborts before any network call. Each request maps to a single image-generation call, correlated by `customId`.
