...
`batch.images()` mirrors [`batch()`](/text): pass an image model and a list of `prompt`s, and it returns the same `BatchJob` handle **immediately**. Each request produces one or more images, correlated by `customId`.
...
`defaults` are merged into every request (request-level values win), exactly as in [`batch()`](/limits#defaults) — handy for applying one `size` or `providerOptions` across the whole batch.
