...
`usage` is normalized to `{ inputTokens, outputTokens, totalTokens }`. Batch pricing is roughly **50% off** synchronous rates on both providers — the discount is applied by the provider, so the tokens you see here are billed at the batch rate.
