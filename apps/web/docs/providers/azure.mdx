...
Batchwork sends API keys as Azure's `api-key` header. For Microsoft Entra ID, pass your own bearer token in `headers`; caller-supplied `Authorization` or `api-key` headers take precedence over environment credentials.
