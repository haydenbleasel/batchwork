...
:::warning xAI batch returns **signed image URLs** on `result.images[].url` (and/or inline `data`) that **expire ~1h** after the batch completes — fetch and persist them promptly. See [Images](/docs/images). :::
