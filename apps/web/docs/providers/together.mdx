...
Together's batch API is chat/audio only — it rejects the embeddings endpoint and offers no batch image generation, so [`batch.embeddings()`](/docs/embeddings) and [`batch.images()`](/docs/images) throw `UnsupportedProviderError`.
