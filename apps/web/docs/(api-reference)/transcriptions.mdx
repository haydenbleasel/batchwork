...
You can also pass a `"provider/model"` string (e.g. `"mistral/voxtral-mini-latest"`) — the only form for Mistral, whose AI SDK package has no transcription model yet.
...
| --- | --- | --- |
| Groq | `whisper-large-v3` | `/v1/audio/transcriptions`, audio by `url`. Translations via `providerOptions`. |
