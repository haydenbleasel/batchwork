---
title: Installation
description: Install Batchwork and the AI SDK provider packages for the providers you batch with — optional peer dependencies that load lazily per provider.
---
...
Azure OpenAI also needs `AZURE_RESOURCE_NAME` (or an explicit Azure `baseURL`) and accepts `AZURE_API_KEY` or `AZURE_OPENAI_API_KEY`. See [Azure OpenAI](/docs/providers/azure) for deployment and Global Batch requirements.
...
- **`UnsupportedProviderError`** — the model resolved to a provider without a batch adapter (i.e. not one of the eight [supported providers](/docs/providers/openai)).
