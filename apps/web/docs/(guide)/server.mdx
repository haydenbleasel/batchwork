...
instances or regions. Durable stores should implement the atomic `claim`
method so only one instance can reserve a webhook id before work continues; the
legacy `get`/`set` methods are serialized within a single process for backward
compatibility.
