...
Batchwork reads the provider and model id from whatever you pass and routes to that provider's native batch endpoint. All requests in a single batch use one model. Which OpenAI endpoint a model maps to — chat completions, responses, or completions — is covered on the [Text](/docs/text#openai-request-shapes) page.
...
:::warning Batch traffic does **not** route through the Vercel AI Gateway (which has no batch endpoint). You need the provider key (e.g. `OPENAI_API_KEY`), not just a gateway key. :::
