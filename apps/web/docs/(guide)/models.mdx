...
Batchwork reads the provider and model id from whatever you pass and routes to that provider's native batch endpoint — each one is covered on its own page: [OpenAI](/docs/providers/openai), [Anthropic](/docs/providers/anthropic), [Google Gemini](/docs/providers/google), [Groq](/docs/providers/groq), [Mistral](/docs/providers/mistral), [Together AI](/docs/providers/together), and [xAI](/docs/providers/xai). All requests in a single batch use one model. Which OpenAI endpoint a model maps to — chat completions, responses, or completions — is covered on the [Text](/docs/text#openai-request-shapes) page.
