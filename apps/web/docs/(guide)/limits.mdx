...
Batchwork validates every batch before it reaches a provider. The default
guardrails are 50,000 requests per batch, 20 MiB per captured request body, a
200 MiB provider upload payload, and 16 concurrent request captures:
...
Provider-side caps still apply, so keep custom limits at or below the target
provider's maximums.
