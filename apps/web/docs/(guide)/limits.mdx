...
`metadata` attaches free-form key/value pairs to the batch. It's forwarded to OpenAI, Azure OpenAI, Groq, Together AI, and Mistral, and ignored by Anthropic, Google Gemini, and xAI (whose batch APIs don't accept batch-level metadata):
...
- **Azure OpenAI** accepts up to 100,000 requests in a 200 MB input file. Batchwork's conservative 50,000-request default still applies; set `limits.maxRequests: 100_000` when your Global Batch or Data Zone Batch deployment and enqueued-token quota allow it. Azure's quota varies by model, deployment type, and subscription — see [Azure's current Batch limits](https://learn.microsoft.com/en-us/azure/foundry/openai/how-to/batch?tabs=global-batch%2Cstructured-outputs%2Cpython-secure&pivots=rest-api#batch-limits).
