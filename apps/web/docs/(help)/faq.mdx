---
title: FAQ
description: Answers to common Batchwork questions — batch turnaround times, blocking, provider SDKs, gateway model strings, tool support, failed requests, and pricing.
---
...
No. `batch()` submits and returns a `BatchJob` handle immediately. Use `job.wait()` for scripts, or the [server layer](/docs/server) for production so you never hold a process open.
...
Yes, as **request inputs**. Batchwork derives each request body through the AI SDK itself (see [Models](/docs/models)), so messages, tools, multimodal content, and `providerOptions` all carry through. It serializes the request only — it does not execute tool calls or parse `generateObject`-style structured output for you, so read the raw payload (tool calls, structured output, finish reasons) from `result.response` and parse it yourself.
...
OpenAI, Anthropic, Google Gemini, Azure OpenAI, Groq, Mistral, Together AI, and xAI today. Beyond chat completions, [embeddings](/docs/embeddings) batches run on OpenAI, Mistral, and Google, and [image generation](/docs/images) batches run on OpenAI, Google, and xAI. Amazon Bedrock and Vertex AI are on the roadmap (they need object-storage staging and cloud IAM auth).
