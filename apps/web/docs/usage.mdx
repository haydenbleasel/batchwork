---
title: Usage
description: Submit a batch, wait for it to finish, and read normalized results — the complete Batchwork workflow from end to end on any provider.
---
...
`wait()` also accepts a `timeoutMs` and an `AbortSignal` via `signal`. For production, prefer the [server layer](/docs/server) over long-lived `wait()` loops.
...
Each [`BatchResult`](/docs/results) carries a normalized `status`, `text`, `usage`, the raw provider `response`, and an `error` when the request failed.
